...
# ========
#
# LiftOverChain is an implementation of a liftOver capable Bio.Intervals.IntervalCollection
#
...
function liftover{T}( chain::LiftOverChain, istream::Bio.Intervals.IntervalStreamOrArray{T}; minidentity=0.95 )
    ...
    while !done(chain, chain_state)
        ...
        if precedes( chain_el, istream_el ) 
            # increment chain
...
liftfirst( i::Bio.Intervals.IntervalValue{Int64,Int64} ) = i.value
liftlast(  i::Bio.Intervals.IntervalValue{Int64,Int64} ) = i.value + (i.last - i.first)

function liftinternal( i::Bio.Intervals.IntervalValue{Int64,Int64}, tolift::Int64 )
...
function Base.collect( chain::LiftOverChain )
    ...
    # a vector of Bio.Intervals.Interval{Bio.Intervals.Interval}() pairs
